...
The app uses Codespaces as the proxy backend only for accounts you add and authorize. For cost control, automatic startup reuses an existing account-owned `proxy*` Codespace before creating anything new: active Codespaces first, stopped Codespaces second, and creation only when no reusable proxy Codespace exists. It skips limited accounts for automatic selection and stops extra active account-owned `proxy*` Codespaces so only one proxy backend remains active.

When storage quota is limited, automatic cleanup deletes stopped account-owned `proxy*` Codespaces to remove storage cost. Manual or unrelated repository Codespaces are not deleted automatically. Set `GitHub__AutoDeleteStorageLimitedProxyCodespaces=false` to disable automatic deletion.

The Codespaces tab includes a Quota Forecast card. It sums remaining compute quota across added accounts with known Free/Pro plans, shows the next GitHub billing reset date, and estimates how many whole days remain by weighting recent app-managed proxy usage from the last 7, 14, and 30 days. When GitHub billing is reachable but returns no Codespaces usage rows for a known Free/Pro account, the account is counted as unused quota. Accounts with unavailable billing usage or unknown plan limits are listed as warnings and are not counted in the aggregate compute pool. The forecast is informational; per-account limited state still controls create/start blocking.
...
6. Send traffic to the proxy port. The backend reuses an existing account-owned `proxy*` Codespace when one is available, creates one only when none exists, starts or attaches to the selected Codespace, opens a hidden dynamic SOCKS tunnel through SSH, starts Xray through that tunnel, and serves the waiting request.
...
The Codespaces tab uses official GitHub REST APIs for normal lifecycle management. Automatic proxy startup opens a hidden local dynamic SOCKS tunnel through the selected Codespace, then runs Xray behind the public gateway port.

By default, startup verifies `gh codespace ssh -c <codespace> true`, generates an OpenSSH config with `gh codespace ssh --config`, then runs a long-lived `ssh -N -D 127.0.0.1:<hidden-port>` tunnel. Xray uses that listener as a SOCKS upstream, so native SSH mode does not use or start the remote proxy on port `8899`, even when `LocalProxy__CodespaceEnsureRemoteProxy=true`. Set `LocalProxy__CodespaceTunnelMode=ports-forward` to use `gh codespace ports forward 8899:<hidden-port> -c <codespace>`, or `LocalProxy__CodespaceTunnelMode=ssh-direct` to open one `ssh -W 127.0.0.1:8899 <codespace-host>` bridge per incoming proxy connection. Those legacy modes continue to use the remote proxy and honor `LocalProxy__CodespaceEnsureRemoteProxy`. `ssh-direct` is useful for diagnostics but should not be the default for whole-VPN traffic because each proxy connection creates a separate SSH bridge. Set `LocalProxy__CodespaceRequireSshReady=false` when the separate readiness command is unreliable but forwarding itself works. If the native SSH tunnel exits unexpectedly, `LocalProxy__CodespaceReconnectOnTunnelExit=true` makes the app reopen the tunnel on the same hidden port without restarting Xray or the public gateway listener; a failed reconnect falls back to the older full retry path. If the idle window is reached, the panel reports `Idle`/`ZzzIdle`; automatic wake is thresholded and the backing Codespace is stopped to save usage.

Large downloads use OpenSSH dynamic forwarding in the default mode, avoiding the remote Python proxy relay. Keep the legacy tunnel modes for diagnostics or compatibility when their remote proxy dependency is acceptable.

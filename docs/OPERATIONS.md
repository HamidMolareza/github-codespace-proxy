...
6. Send traffic to the proxy port. The backend reuses an existing `wproxy97/proxy2` Codespace when one is available, creates one only when none exists, starts or attaches to the selected Codespace, opens a hidden dynamic SOCKS tunnel through SSH, starts Xray through that tunnel, and serves the waiting request.

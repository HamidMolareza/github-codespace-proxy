...
The active workflow manages GitHub username/PAT records and runs a proxy through the selected GitHub Codespace. One Run action starts or resumes the Codespace, verifies the remote proxy on `127.0.0.1:8899`, opens an OpenSSH tunnel, starts Xray, and exposes one local mixed HTTP/SOCKS port.

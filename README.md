...
The default tunnel mode uses:

```bash
gh codespace ports forward 8899:<hidden-port> -c <codespace>
```

VPS deployments can set `LocalProxy__CodespaceTunnelMode=native-ssh` to generate an OpenSSH config with `gh codespace ssh --config` and run a native `ssh -N -L` tunnel.
